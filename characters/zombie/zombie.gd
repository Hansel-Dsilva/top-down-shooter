...
export (float) var attack_rate  # delay time (s) between attacks
...
func _process(delta):
	...
	#non-path chase
	if target and los and abs((target.global_position - global_position).length()) > 60:
		set_process(true)
...
#	elif not los and player_last_pos and abs((position - player_last_pos).length()) > 10:
##		target = null
##		los = false
...
func move_along_path(delta, distance : float):
...
#			look_at(path[0])
			if distance_to_next:
				next_posn = start_point.linear_interpolate(path[0], distance / distance_to_next)
# warning-ignore:return_value_discarded
#				move_and_collide(next_posn - global_position)
				velocity = position.direction_to(next_posn) * MAX_SPEED
				velocity = move_and_slide(velocity)
#				move_and_slide(((next_posn - global_position).normalized() * MAX_SPEED / delta))
...
#			move_and_slide((next_posn - global_position) * 0)
#			set_process(false)
			break
...
#	for pos in [target.position, nw, ne, se, sw]:
	result = space_state.intersect_ray(global_position, target.position, [self], 3)
	if result:
...
func move_to_last_seen(delta):
	emit_signal("player_spot", self, player_last_pos)
	var move_distance = MAX_SPEED * delta
	move_along_path(delta, move_distance)
	if not target:
		set_process(false)
